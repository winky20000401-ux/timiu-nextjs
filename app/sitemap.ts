...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return [
    ...
    ...visibleArticles.map((article) => ({ url: `${base}/article/${article.slug}`, lastModified: new Date(article.updatedAt), changeFrequency: "weekly" as const, priority: article.featured ? 0.9 : 0.7 })),
    ...visibleTags.map((tag) => ({ url: `${base}/tag/${encodeURIComponent(tag)}`, lastModified: now, changeFrequency: "weekly" as const, priority: 0.5 })),
  ];
}
